...
보정량을 현재 자세에 더하면, 다음 사이클의 오차는 직전 오차의 (1 − gain)배가 된다. 즉 오차가 사이클마다 일정 비율로 줄어드는 기하급수적 수렴(geometric/exponential convergence)이다.

$$e_{k+1} = (1 - g)\,e_k$$

초기 오차 9.84 mm, gain = 0.6일 때 이론상 수렴 경로는 다음과 같다.
...
gain을 0.6으로 잡은 것은 속도와 안정성의 절충이다. gain이 1.0에 가까울수록 한두 사이클 만에 목표에 닿지만, D2처럼 측정에 노이즈가 많은 환경에서는 보정 명령이 과하게 출렁여(오버슈트) 수렴이 불안정해진다. 0.6은 4사이클 안에 허용오차에 들면서도 노이즈 환경에서 댐핑 안정성을 확보하는 값이다. 실제 실험에서도 보정 사이클은 대부분 2~4회 사이에서 끝났다.
...
여기서 scale $s$(픽셀당 실제 거리, 통상 GSD라 부른다)와 원점 $(u_0, v_0)$는 고정 상수가 아니라 **카메라·해상도에 의존하므로 런타임에 계산한다**. 같은 매핑을 640×480 개발·검증 환경에서 측정하면 $s \approx 0.002893$ m/px, $u_0 \approx 215.8$, $v_0 \approx 239.6$이었고(`interface_contract.md`), 1280×960 캠페인에서는 그 해상도에서 다시 캘리브한 값을 쓴다. D3 드리프트가 위험한 이유가 이 식에서 드러난다 — 이 변환은 카메라 좌표계가 고정되어 있다고 전제하는데, 마운트가 움직이면 $(u_0, v_0)$ 기준 자체가 틀어져 변환식이 오염된다.
...
비전 모듈이 구현되기 전에도 이 배관이 제대로 도는지 먼저 확인하기 위해, 사이클이 늘수록 오차를 0.6배씩 줄이는 더미 인지(Dummy Perception) 로직으로 통신 신뢰성을 독립 검증했다(§부록 B, S2).
...
고른 이유는 두 가지다. 첫째, Panda는 정밀 조작·조립 연구에서 학계와 산업계가 함께 쓰는 표준 참조 플랫폼이라 결과를 기존 연구와 바로 견줄 수 있다. 둘째, 공식 Menagerie 모델이 있어 커스텀 URDF 없이 검증된 동역학 파라미터를 그대로 쓸 수 있고, 그만큼 모델링 오차가 적다. (과제 제약상 커스텀 로봇 모델 작성이 금지되어 있어 공식 모델만 쓴 것도 이 선택과 맞물린다.)
...
원본 모델에는 카메라가 없어(ncam = 0), 코드에서 elevation = −90°의 top-down 수직 카메라를 동적으로 추가했다. 렌더 해상도는 두 가지를 쓴다 — 개발·검증 단계의 self-test는 640×480으로 빠르게 돌리고, 실제 실험 캠페인은 정밀 측정을 위해 1280×960 오프스크린 렌더러를 쓴다(§2.3). 또 중력 때문에 제어가 없을 때 팔이 처져 관측이 왜곡되는 것을 막기 위해, 매 렌더 직전에 `home` 키프레임의 관절 각도를 강제로 주입해 자세를 고정했다.

구현 중에 만난 까다로운 문제 하나는 한글 경로였다. 사용자명에 비ASCII 문자(한글)가 들어 있어 MuJoCo C++ 내부의 `fopen`이 유니코드 경로를 열지 못했고, `cv2.imwrite`도 한글 경로에서 실패했다. 모델은 디렉터리로 `os.chdir` 후 ASCII 파일명만 넘기는 방식으로, 이미지 저장은 `cv2.imencode`로 메모리에 인코딩한 뒤 파이썬 파일 IO로 바이트를 직접 쓰는 방식으로 우회했다. 문서를 보고 따라 한 게 아니라 실제로 돌리다 막혀서 푼 문제라는 점에서, 이 디버깅 자체가 구현의 한 결과물이다.
...
D1·D2로 마커가 손상되면 Contour Backup이 켜진다. OTSU로 이진화한 뒤 OpenCV 윤곽선 검출을 수행하고, 가장 큰 윤곽의 무게중심을 $(x, y)$로, 타원 피팅의 장축 방향을 $\theta$로 근사한다. ArUco보다 정밀도는 낮지만, 마커가 완전히 깨진 상황에서도 검출을 이어가게 해 준다. 두 경로 모두 실패하면 `DetectionError`를 던지고 해당 시도를 ABORT로 기록한다. 이 예외가 시스템 전체를 멈추지 않고 다음 시도로 넘기도록 설계해, 장시간 자동 실험의 안정성을 확보했다.
...
허위 수렴은 바로 이 구조에서 싹튼다. **M2의 계산 자체는 늘 정확하다.** 문제는 입력이다 — M1이 오염된 캘리브레이션 기준으로 잰 `estimated_pose`를 넘기면, M2는 "틀어진 자로 잰 오차"만 0으로 수렴시킨다. 측정 오차는 깔끔히 줄어드는데 실제 물리 오차는 그대로 남는 구조적 원인이 여기 있다. 이 현상의 크기는 §8.3에서 정량화한다.
...
허용오차는 데이터가 정해 준 값이 아니라 **내가 설계 단계에서 고른 값**이다(`run_cycle.py`: `SRS_TOL = {"xy": 5e-4, "theta": radians(2.0)}`, 즉 위치 ±0.5 mm / 각도 ±2°). 중요한 것은 숫자 자체가 아니라 왜 이 값을 골랐는가다.

위치 ±0.5 mm를 고른 근거는 두 가지다. 첫째, 광학 하우징 지그의 기계적 클리어런스를 고려했다 — 이보다 크게 어긋나면 부품과 하우징이 물리적으로 간섭한다. 둘째, 측정 가능성과 균형을 맞췄다. 캘리브된 픽셀 스케일에서 한 픽셀이 차지하는 실제 거리가 약 1 mm 안팎이므로, 그 절반 수준인 ±0.5 mm는 서브픽셀 정밀도 범위에서 비전이 현실적으로 도달할 수 있는 하한이다. 더 빡빡한 기준(예: ±0.1 mm)을 걸면 노이즈 환경에서 허용오차에 영영 못 들어 보정 루프가 무한 반복에 빠질 수 있다 — 이건 실제로 더 작은 tolerance를 시도하며 확인한 trade-off다.
...
요컨대 ±0.5 mm / ±2°는 "완벽한 정밀도"를 주장하는 값이 아니라, **기계적 제약·센서 분해능·수렴 가능성의 교집합에서 합리적으로 타협한 설계 결정**이다. 그리고 바로 이 기준이 있었기에, D3에서 측정은 기준 안에 들지만 실제는 한참 벗어나는 허위 수렴을 정량적으로 드러낼 수 있었다.
...
| S4 Loop | 보정 폐루프 연동 | run_cycle.py 상태 기계, gain=0.6 반복 보정 | 보정 전후 오차 감소 확인 |
...
실험은 두 가지를 본다. 첫째, 비전 보정 루프가 외란 없는 기준 조건에서 오차를 얼마나 줄이는지 정량화한다. 둘째, 외란의 종류와 강도가 세질 때 어느 지점에서 비전 측정이 실패하거나 허위 수렴이 발생하는지 짚어낸다. 둘을 합치면 시스템이 정상 작동하는 한계를 그릴 수 있다.

### 7.2 실험 요인 설계

실험은 완전 교차 격자(Full-Factorial Grid) 설계를 따른다. 단, 외란끼리 섞여 오염되는 것을 막기 위해 한 번에 외란 하나만 켠다 — D1을 켤 때 D2·D3는 off로 둔다.
...
조건당 8회 반복은 이진 결과(성공/실패)에 대해 95% 신뢰구간 반폭 약 ±0.35의 추정 정밀도를 준다. 비례 보정의 수렴이 결정론적이라 외란 없는 조건에서는 분산이 작고, 외란이 있는 조건에서는 랜덤 시드에 따른 변동을 잡아내기에 충분한 수준이다. (더 큰 표본이 통계적 엄밀성을 높일 수 있다는 점은 한계로 남는다 — §9.2.)
...
보정 OFF에서는 8 trial 모두 측정 잔류 오차 9.836 mm, Ground-Truth 오차 10.000 mm로 허용오차(±0.5 mm)를 크게 넘겨 측정·실제 성공률 모두 0%였다. 보정 ON에서는 측정 잔류 오차 0.381 mm, GT 오차 0.173 mm로 수렴해 두 성공률 모두 100%였다.

측정 기준 오차 감소비는 9.836 / 0.381 = **약 26배**다(측정값 대 측정값의 like-for-like 비교라 이 값을 대표로 쓴다). Ground-Truth 기준으로 보면, 주입한 시작 오프셋 10.000 mm(placement error +8, −6 mm의 norm)에서 보정 후 0.173 mm까지 내려가 약 58배에 해당한다 — 다만 이 분모는 측정이 아니라 실험이 정의한 시작값이므로, 26배와 동일한 성격의 지표는 아니다. 두 수치 모두 비례 제어 루프가 외란 없는 조건에서 목표 정밀도에 안정적으로 수렴함을 보여준다. 외부 비전 보정이 없으면 누적 기계 오차로 1 cm 가까이 벌어지지만, 보정을 켜는 것만으로 그 오차가 흡수된다 — 외부 모듈이 존재하는 가치가 여기 있다.
...
**D3 strong의 극단적 사례.** 8 trial 전체에서 측정 잔류 오차는 0.12~0.482 mm로 허용오차 안이지만, GT 오차는 1번 trial 0.173 mm에서 8번 trial 23.191 mm까지 퍼진다. 평균으로 보면 측정 0.296 mm 대 GT 11.623 mm, 비율은 **약 39배**다.[^drift] 메커니즘은 코드에서 분명하다 — D3는 카메라 `lookat`을 사이클에 비례해 옮긴다(`disturbance.py`: strong = `3e-3` m/cycle, y축은 ×0.5). 보정 루프가 수렴하는 데 걸린 사이클 수가 trial마다 다르므로 누적 드리프트도 trial마다 달라지고, 그래서 GT 오차가 0.17~23 mm로 넓게 퍼진다. 비전 모듈은 이 드리프트를 감지하지 못한 채 오염된 기준으로 잰 "0에 가까운 오차"를 보고한다. 실제로는 드리프트가 클수록 GT 오차가 쌓이지만, 시스템은 그것을 모른다.

[^drift]: 참고로 `interface_contract.md`에는 640×480 검증 환경에서 strong 드리프트가 cycle=10일 때 마커 중심을 약 11.63 **px** 이동시킨다는 실측이 있다. 본문의 11.62 **mm**(1280×960 캠페인의 D3 strong 평균 GT 잔류오차, `summary.csv`)와 숫자 크기가 비슷하지만 단위가 다른 별개의 값이다 — 전자는 픽셀 이동량, 후자는 물리 오차다. 같은 수를 재사용한 것이 아니라, 동일한 드리프트 강도에서 우연히 크기가 가까운 것이다.

![그림 4. 비례 제어 수렴 특성(왼쪽)과 D3 드리프트 조건의 허위 수렴(오른쪽). 왼쪽은 gain별 이론 수렴 경로(gain=0.6이면 4사이클 안에 허용오차 진입). 오른쪽은 측정 오차는 ±0.5 mm 아래로 내려가지만 Ground-Truth 오차는 11.6 mm에 고정되는 패턴을 보여준다.](report/G5_convergence.png)
...
![그림 5. 보정 ON 조건의 측정 잔류 오차(파란 막대) vs. Ground-Truth 잔류 오차(빨간 막대). D3 strong에서 시스템은 0.30 mm를 보고하지만 실제는 11.62 mm로, 약 39배 차이다. 보라색 점선은 SRS 허용오차 ±0.5 mm.](report/G2_residual_gap.png)

### 8.4 외란 내성 서열

세 외란의 영향을 S_gt 기준으로 정리하면 이렇다.

**D2(센서 노이즈)**는 weak·med에서 완전히 성공하고 strong에서만 부분 실패한다. 가우시안 노이즈는 ArUco 코너 추정 오차를 키우지만 기준 자체를 오염시키지는 않아, 강도가 낮으면 비전의 내재적 필터링 능력으로 흡수된다.

**D1(조명)**은 강도와 무관하게 측정 기준을 일정 오프셋으로 오염시켜 모든 수준에서 허위 수렴을 부른다. 오프셋이 허용오차와 가까워 경계 효과가 뚜렷하다.

**D3(마운트 드리프트)**는 강도가 오를수록 GT 오차 평균이 1.803 → 5.688 → 11.623 mm로 가파르게 악화되는 반면, 측정 오차는 강도와 무관하게 ±0.5 mm 안으로 수렴한다. 비전이 카메라 좌표계 자체가 움직였다는 사실을 감지할 수단이 없기 때문이다. 세 외란 중 D3가 가장 치명적이다.

![그림 6. 전체 160 trial의 사이클 소요 시간 대 측정 잔류 오차 분포. 보정 OFF(회색)는 0.08 s 부근으로 빠르지만 오차가 10 mm에 달해 쓸 수 없고, 보정 ON(초록)은 0.2~1.1 s의 추가 시간 비용으로 잔류 오차를 SRS 허용오차(±0.5 mm) 근처로 낮춘다. 단, 노이즈(D2)가 심해지면 수렴에 필요한 사이클이 늘어 시간이 길어지는 트레이드오프가 보인다.](report/G4_tradeoff.png)
...
지금 구현은 2D 평면 자세 $(x, y, \theta)$만 추정한다. 렌더러가 Depth 프레임을 함께 내지만 M1은 이를 쓰지 않는다. 실제 광학 탑재체 AIT 공정에서는 광축 방향 평행 이동(Z축)과 피치·롤 회전까지 포함한 6-DOF 정렬이 필요하다. 이를 넓히려면 스테레오 비전이나 구조광 기반 3D 자세 추정이 있어야 한다.

### 9.2 실험 설계의 한계

실험은 외란을 한 번에 하나씩 거는 단일 요인 설계를 따랐다. 실제 현장에서는 조명(D1)과 노이즈(D2)가 동시에 발생하고, 마운트 드리프트(D3)가 배경으로 늘 깔려 있는 복합 외란이 일반적이다. 복합 외란에서 허위 수렴 빈도와 외란 간 상호작용을 재는 실험은 향후 과제다.

표본 크기도 한계다. 조건당 8회는 과제 수준에서는 충분하지만, 통계적 엄밀성을 더 높이려면 반복 수를 늘려야 한다. 또 D3의 GT 오차가 사이클당 고정 스텝(`3e-3` m/cycle)으로 더해지는 discrete step 모델이라, 실제처럼 연속적으로 드리프트가 쌓이는 모델로 확장하는 것도 남은 일이다.

시뮬레이션 자체의 결정론적 성격 역시 한계다. MuJoCo 안에서 로봇 동역학은 노이즈 없이 정확히 계산되며, 실제 하드웨어의 관절 마찰·모터 토크 분산·엔드이펙터 진동이 반영되지 않는다.

### 9.3 Sim-to-Real 전환 격차

§4.3에서 부품을 mocap body로 추상화한 결과, 실제 Franka Panda 하드웨어에서 생기는 관절 마찰·모터 전류 분산·엔드이펙터 진동이 현재 시뮬레이션에 빠져 있다. 그리퍼가 부품을 쥘 때 가하는 힘이 부품 자세를 미세하게 바꿀 수도 있다. 이런 오차 원인이 쌓이면 실제 정밀도는 시뮬레이션 수치보다 낮아진다. 따라서 §8의 성공률과 잔류 오차는 실제 하드웨어에서의 상한으로 읽어야 한다.

### 9.4 향후 연구 방향

먼저 다룰 과제는 세 가지다. 첫째, D3 대응이다. 씬에 고정 참조 마커를 추가해 카메라 좌표계 자체의 드리프트를 실시간으로 추정·보정하는 자동 재캘리브레이션을 넣으면, 허위 수렴의 근본 원인인 기준 오염을 사전에 차단할 수 있다. 둘째, D1·D2·D3를 동시에 거는 복합 외란 실험으로 현실에 더 가까운 조건에서 신뢰도를 측정한다. 셋째, 실제 Panda 하드웨어에서 같은 보정 루프를 돌려 Sim-to-Real 격차를 정량화하고, 필요하면 마찰 보상이나 그리퍼 힘 제어를 더한다.
...
첫째, 비례 제어 보정 루프는 외란 없는 기준 조건에서 측정 잔류 오차를 약 26배(9.84 → 0.38 mm) 줄이고, 주입한 10 mm 시작 오프셋 기준으로는 Ground-Truth 오차를 0.173 mm까지 끌어내리며 조립 성공률을 0%에서 100%로 올린다. 비전 기반 반복 보정이 광학 부품 정밀 정렬 자동화에 실효가 있음을 정량적으로 뒷받침한다.

둘째, 세 가지 현실적 외란(D1 조명·글레어, D2 센서 노이즈, D3 마운트 드리프트) 모두에서 허위 수렴이 공통으로 나타난다. 시스템이 내부적으로 "성공"을 보고하는 동안 실제 물리 오차는 수렴하지 않는 이 현상은 D3 strong에서 측정 0.296 mm 대 GT 11.623 mm(약 39배)로 가장 극단적으로 드러난다. 측정 성공률만을 신뢰도 지표로 쓰면 치명적인 오정렬을 "정렬 완료"로 오판할 수 있다.

실용적 함의는 분명하다. 비전 기반 폐루프 정렬 시스템은, 측정 기준 자체의 무결성을 독립적으로 검증하는 이중 지표 구조를 갖추거나, 외부 캘리브레이션 참조 없이는 드리프트를 감지할 수 없는 구조적 약점을 메우는 재캘리브레이션 장치를 갖추어야 한다. 광학 부품처럼 µm 정밀도가 요구되는 맥락에서는, 비전의 자기 측정을 외부 절대 기준으로 교차 검증하는 것이 안전성의 전제 조건이다.
...
[5] K. J. Åström and B. Wittenmark, *Computer-Controlled Systems: Theory and Design*, 3rd ed. Mineola, NY: Dover Publications, 2011.
...
본 과제의 시뮬레이션 시스템은 AI 에이전트(Claude Code)를 활용한 구조화된 단계별 제작 방식으로 개발했다. 이 부록은 그 방법을 기록한다.

### B.1 제작 루프 설계 원칙

각 단계는 P1(계획) → P2(코드 작성 + 실행) → P3(정량 평가) → P4(피드백)의 4단계 루프로 진행했다. 원칙은 두 가지다. 첫째, 각 프롬프트는 이전 대화 맥락에 기대지 않도록 필요한 입력을 프롬프트 안에 모두 넣는다. 둘째, P2는 코드 작성에서 멈추지 않고 실제 이 PC에서 돌린 뒤 콘솔 로그와 산출물을 함께 제출한다 — 실행하지 않은 상태로는 P3 평가를 통과할 수 없게 설계했다. P3는 배점 기반 체크리스트로 진행하고, 합격선에 못 미치면 P4 피드백으로 P2에 되돌아가며, 단계당 최대 3회 반복 후 한계를 기록하고 다음 단계로 넘어간다.

### B.2 단계별 요약

전체 구현은 S1~S7 7단계로 나눴다. S1(환경 부팅): MuJoCo 씬 로드와 카메라 렌더. S2(연동 배관): JSON 인터페이스 검증. S3(비전 모듈): ArUco 검출·GSD 캘리브·M2 보정 벡터. S4(보정 폐루프): run_cycle.py 상태 기계. S5(외란 주입): D1·D2·D3 파라미터화 인가. S6(실험 캠페인): 160 trial 자동 스윕과 CSV 누적. S7(분석): make_figures.py 그래프 5종.

### B.3 환경 특이사항과 해결책

개발 환경(Windows 11, Python 3.13, MuJoCo 3.10)에서 두 가지 기술 문제가 있었다.

**비ASCII 경로 문제.** 사용자명에 한글이 들어 `mujoco.MjModel.from_xml_path(절대경로)`와 `cv2.imwrite(한글경로, img)`가 실패했다. 모델 로드는 `os.chdir`로 모델 디렉터리로 이동한 뒤 ASCII 파일명만 넘기고, 이미지 저장은 `cv2.imencode`로 메모리에 인코딩한 뒤 파이썬 파일 IO로 바이트를 직접 기록해 우회했다.

**부품 모델 추상화.** 광학 부품을 커스텀 MJCF로 만들지 않고 mocap body로 모델링해 실험 제어를 쉽게 했다. 이 선택의 Sim-to-Real 함의는 §9.3에서 다룬다.

### B.4 외란 모델 파라미터 (코드 출처)

세 외란의 강도 수치는 `sim/disturbance.py`의 `LEVELS` 딕셔너리에 한곳으로 고정해 재현성을 확보했다. off 조합은 baseline과 비트 단위로 동일함을 self-test에서 검증한다(항등성). 적용 순서는 D3(카메라 이동) → D1(조명·글레어) → D2(노이즈)다.
...
D3는 다른 둘과 성격이 다르다. D1·D2는 이미지 픽셀을 흔들 뿐 캘리브레이션 기준은 건드리지 않는 반면, D3는 카메라 외부 파라미터(`lookat`) 자체를 사이클에 비례해 이동시켜 고정 캘리브 기준 대비 측정 bias를 만든다. `interface_contract.md`의 표현을 빌리면 "고정 calib 대비 측정 bias = 드리프트의 정의"다. 이것이 D3가 측정값은 깨끗한데 실제 오차만 누적되는 허위 수렴을 일으키는 근본 원인이다.


### B.4 JSON 인터페이스 계약

시뮬레이터와 외부 비전 모듈의 통신은 다음 스키마를 따른다.
