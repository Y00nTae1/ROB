...
![그림 1. 비전 보정 폐루프 구조. M1 자세 추정 → M2 편차·보정량 계산 → 부품 이동 → 허용오차 판정 순으로 반복하고, 판정을 통과하면 정렬을 완료한다.](proportional_correction_loop_mechanism.png)
...
![그림 2. 기준 조건(외란 없음)에서 보정 OFF/ON 비교. 왼쪽: 조립 성공률 0% → 100%. 오른쪽: 측정 잔류 오차 9.84 mm → 0.38 mm(약 26배 감소). 보정 ON 시 잔류 오차가 SRS 허용오차(±0.5 mm) 아래로 내려간다.](G3_correction_value.png)
...
![그림 3. 세 외란 조건별 측정 성공률(파란 실선) vs. 실제 성공률(빨간 점선). D1은 전 강도에서 100%p, D2 strong에서 약 38%p, D3는 전 강도에서 약 88%p의 괴리가 벌어진다. 주황색 음영이 곧 허위 수렴 영역이다.](G1_false_convergence.png)
...
![그림 4. 비례 제어 수렴 특성(왼쪽)과 D3 드리프트 조건의 허위 수렴(오른쪽). 왼쪽은 gain별 이론 수렴 경로(gain=0.6이면 4사이클 안에 허용오차 진입). 오른쪽은 측정 오차는 ±0.5 mm 아래로 내려가지만 Ground-Truth 오차는 11.6 mm에 고정되는 패턴을 보여준다.](G5_convergence.png)
...
![그림 5. 보정 ON 조건의 측정 잔류 오차(파란 막대) vs. Ground-Truth 잔류 오차(빨간 막대). D3 strong에서 시스템은 0.30 mm를 보고하지만 실제는 11.62 mm로, 약 39배 차이다. 보라색 점선은 SRS 허용오차 ±0.5 mm.](G2_residual_gap.png)
...
![그림 6. 전체 160 trial의 사이클 소요 시간 대 측정 잔류 오차 분포. 보정 OFF(회색)는 0.08 s 부근으로 빠르지만 오차가 10 mm에 달해 쓸 수 없고, 보정 ON(초록)은 0.2~1.1 s의 추가 시간 비용으로 잔류 오차를 SRS 허용오차(±0.5 mm) 근처로 낮춘다. 단, 노이즈(D2)가 심해지면 수렴에 필요한 사이클이 늘어 시간이 길어지는 트레이드오프가 보인다.](G4_tradeoff.png)
