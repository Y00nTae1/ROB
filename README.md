...
**저자:** 윤태원 | **제출일:** 2026-06-28 | TelePIX SpaceLAB Fellowship — 로봇 시뮬레이션·자동화 트랙

![정렬 데모 시뮬레이션](report/figures/video/cycle_demo.gif)
...
![그림. 로봇 기반 광학 부품 정렬 자동화 개요](report/1.1.png)
...
![그림. 시뮬레이터와 비전 모듈 분리 구조](report/3.1.png)
...
1. **초기화** — MuJoCo 씬을 로드하고 ArUco 마커를 붙인 부품을 mocap body로 배치하며 카메라와 렌더러를 구성한다.
2. **외란 인가** — 해당 시도의 실험 조건에 따라 D1·D2·D3 중 하나를 적용한다.
3. **영상 캡처** — 1280×960 RGB와 Depth 프레임을 렌더한다.
4. **자세 추정(M1)** — 외부 비전 모듈이 이미지에서 $(x, y, \theta)$를 산출한다.
5. **편차 계산(M2)** — 목표 자세와의 차이를 구하고 보정량을 만든다.
6. **보정 이동** — 보정량을 부품 자세에 더해 씬을 갱신한다.
7. **허용오차 검증** — 다시 찍어 측정 오차가 ±0.5 mm / ±2.0° 안인지 확인한다. 들면 루프를 끝내고 그리퍼를 열어 사이클을 완료하고, 못 들면 최대 10회까지 자세 추정으로 돌아간다. 10회를 넘기거나 `DetectionError`가 나면 ABORT로 전환한다.
8. **로깅** — 측정 잔류 오차, Ground-Truth 오차, 사이클 수, 소요 시간을 CSV에 기록한다.
...
| 패키지 | 버전 | 용도 |
|---|---|---|
| mujoco | 3.10.0 | 물리 시뮬레이터 엔진 |
| numpy | 2.5.0 | 수치 연산 |
| opencv-python | 4.13.0 | ArUco 검출, 이미지 처리 |

### C.2 MuJoCo Menagerie 모델 준비

Franka Panda 모델은 MuJoCo Menagerie에서 가져온다. 레포지토리의 `mujoco_menagerie/franka_emika_panda/scene.xml` 경로를 환경변수로 지정한다.

```bash
# Windows (CMD)
set PANDA_XML=C:\...\mujoco_menagerie\franka_emika_panda\scene.xml

# Windows (PowerShell)
$env:PANDA_XML = "C:\...\mujoco_menagerie\franka_emika_panda\scene.xml"

# macOS / Linux
export PANDA_XML=/path/to/mujoco_menagerie/franka_emika_panda/scene.xml
```
...
# 단일 정렬 사이클 (보정 ON, 외란 없음)
python sim/run_cycle.py
```

### C.4 전체 실험 캐페인 (160 trial)

```bash
# 전체 외란 스위프 실행 (기준 + D1/D2/D3 × weak/med/strong × 보정 ON/OFF × 8회 반복)
python experiments/run_experiment.py --disturb none --correction both --trials 8
...
결과는 `experiments/results.csv`에 누적되며, `experiments/summary.csv`에 조건별 요약이 생성된다.
...
실험 CSV를 집계해 `report/` 디렉터리에 그래프 5종(G1–G4 + 보정값 비교)을 생성한다.

### C.6 한글 경로 참고사항

Windows 사용자명에 한글이 포함된 경우, MuJoCo C++ 내부의 `fopen`과 `cv2.imwrite`가 유니코드 경로를 열지 못할 수 있다. 본 프로젝트는 이를 우회하도록 구현되어 있으므로 추가 설정 없이 그대로 실행하면 된다.
