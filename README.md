...
![그림 1. 비례 제어 기반 비전 보정 폐루프 다이어그램. M1 자세 추정 → M2 편차·보정량 계산 → 부품 이동 → 허용오차 판정의 순서로 반복되며, 판정 통과 시 정렬 완료된다.](report/proportional_correction_loop_mechanism.png)
...
![그림 2. 기준 조건(외란 없음)에서 보정 OFF/ON 비교. 왼쪽: 조립 성공률 0% → 100%. 오른쪽: 측정 잔류 오차 9.84 mm → 0.38 mm(26배 감소). 보정 ON 시 잔류 오차가 SRS 허용오차(±0.5 mm) 이하로 진입한다.](report/G3_correction_value.png)
...
![그림 3. 비례 제어 수렴 특성(좌) 및 D3 드리프트 조건에서의 허위 수렴(우). 왼쪽은 gain별 이론 수렴 경로(gain=0.6 기준 4사이클 내 허용오차 진입). 오른쪽은 측정 오차는 ±0.5 mm 이하로 수렴하지만 Ground-Truth 오차는 11.6 mm에 고정되는 패턴을 보인다.](report/G5_convergence.png)
...
![그림 4. 세 외란 조건별 측정 성공률(파란 실선) vs. Ground-Truth 성공률(빨간 점선). D1은 강도 전 구간에서 100% 갭, D2 strong에서 38% 갭, D3는 강도 전 구간에서 88% 갭이 발생한다.](report/G1_false_convergence.png)

![그림 5. 보정 ON 조건의 측정 잔류 오차(파란 막대) vs. Ground-Truth 잔류 오차(빨간 막대). D3 strong에서 시스템은 0.30 mm를 보고하지만 실제 오차는 11.62 mm다(39배 차이). 보라색 점선은 SRS 허용오차 ±0.5 mm.](report/G2_residual_gap.png)
...
![그림 6. 전체 160 trial의 사이클 소요 시간 대비 측정 잔류 오차 분포. 보정 OFF(회색)는 빠르지만 오차가 크고, 보정 ON(초록)은 추가 사이클 시간 비용으로 잔류 오차를 SRS 허용오차(±0.5 mm) 이하로 낮춘다.](report/G4_tradeoff.png)
