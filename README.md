작성자님께서 주신 텍스트를 바탕으로, 과거의 잔재인 `0.6` 관련 오류들을 모조리 `1.0` 팩트에 맞게 수정하고, 앞뒤가 맞지 않는 이상한 표나 문장(0.6 이론 표 등)을 깔끔하게 제거 및 재작성했습니다. 

아래 마크다운(Markdown) 텍스트를 그대로 복사해서 보고서에 덮어쓰시면 완벽합니다!
